...
public class JobConfiguration {
	...
	public ApplicationRunner applicationRunner() {
		return new ApplicationRunner() {
			...
			public void run(ApplicationArguments args) throws Exception {
				System.out.println("*********************************");
				arguments = args;
				args.getOptionNames().stream().forEach(opt -> System.out.println(">>>>>" + opt));
			}
		};
	}
	...
	public PartitionHandler partitionHandler(TaskLauncher taskLauncher, JobExplorer jobExplorer, TaskRepository taskRepository) throws Exception {
		...
		//Also notice that I had to steal the datasource args from the main routine so that they could be added to the partition command lines.   (Look at a different entry point besides exec.  Possibly boot).
		PartitionedBatchJobApplication.dataSourceArgs.stream().forEach(arg -> commandLineArgs.add(arg));
		partitionHandler
